...
SceneBoss::SceneBoss() : Scene(SceneId::Dev1)
{
...
void SceneBoss::Init()
{
	...
	sceneName->text.setString(L"Boss Room");
...
{
	Scene::Enter();
	SpawnMonsters(0, sf::Vector2f(0, 0));
...
void SceneBoss::SpawnMonsters(int count, sf::Vector2f center)
{
	for (int i = 0; i < count; i++)
	{
...
void SceneBoss::OnDieMonster(Monster* monster)
{
	RemoveGo(monster);
...
void SceneBoss::GetCoin(std::vector<SpriteGo*> coin)
{
	coin;
}

const std::list<Monster*>* SceneBoss::GetMonsterList() const
{
	return &poolMonsters.GetUseList();
}

void SceneBoss::NextScene()
{
	if (monsterCount == 0)
	{
		nextScene = true;
